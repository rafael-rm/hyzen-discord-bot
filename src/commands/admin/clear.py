...
class ClearCommand(commands.Cog):
    ...
    @app_commands.describe(quantidade='Quantidade de mensagens a serem excluídas.',
                           usuario='Usuário que terá as mensagens excluídas.')
    ...
    async def clear(self, interaction: discord.Interaction, quantidade: int, usuario: discord.Member = None):
        ...
        if quantidade > 500:
            ...
            async for mensagem in interaction.channel.history(limit=quantidade + 1):
                if ((
                        interaction.created_at - mensagem.created_at).days <= 14 and interaction.created_at > mensagem.created_at):
                    if usuario is None:
            ...
            if len(mensagens_apagar) == 0:
                await interaction.followup.send(
                    content="Nenhuma mensagem foi excluída deste canal, pois todas as mensagens são mais antigas que **14** dias.")

            if len(mensagens_apagar) != quantidade:
                await interaction.channel.delete_messages(mensagens_apagar)
                await interaction.followup.send(
                    content=f"Apenas **{len(mensagens_apagar)}** mensagens foram excluídas deste canal, pois as outras foram enviadas a mais de **14** dias.")
            else:
                await interaction.channel.delete_messages(mensagens_apagar)
                await interaction.followup.send(
                    content=f"**{len(mensagens_apagar)}** mensagens foram excluídas deste canal.")
    ...
    async def erros(self, interaction: discord.Interaction, error):
        ...
        if isinstance(error, app_commands.MissingPermissions):
            ...
            await interaction.response.send_message(
                "A aplicação não tem permissões suficientes para executar esse comando, verifique se a permissão de `Gerenciar Mensagens` está ativada.",
                ephemeral=False)
        else:
